// ============================================================
// CRITERION AMAZING COLLEGE — MAIN APP (Web Service Edition)
// ============================================================
  ...
  <div class="login-screen">
    <div class="card login-card">
      <div class="login-logo">
        ${logoHTML}
        <div class="login-school">CRITERION AMAZING COLLEGE</div>
...
function renderSidebar() {
  ...
  <aside class="sidebar">
    <div class="sidebar-logo">
      ${logoImgHTML}
      <div>
        <div class="sidebar-school">Criterion Amazing College</div>
...
<head>
  <meta charset="UTF-8" />
  <title>${classId} — Student List — Criterion Amazing College</title>
  <style>
...
<div class="page-wrap">
  ...
  <div class="school-header">
    ...
    <div class="school-info">
      <div class="school-name">Criterion Amazing College</div>
  ...
  <div class="doc-footer">
    ...
    <div style="font-size:11px;color:#bbb;text-align:center;">
      Criterion Amazing College — Result Management System<br/>
  ...
  win.document.close();
  win.document.title = classId + ' — Student List — Criterion Amazing College';
}
...
function printAllClasses() {
  ...
  win.document.title = 'Criterion Amazing College - All Results - ' + settings2.session + ' ' + settings2.term;
  setTimeout(() => win.print(), 700);
}
...
// ============================================================
// BURSARS MANAGEMENT (Admin only)
// ============================================================
function renderBursars() {
  const bursars = DB._bursarsList || [];
  if (!DB._bursarsLoaded) {
    DB._bursarsLoaded = true;
    DB.getBursars().then(list => {
      DB._bursarsList = list || [];
      render();
    });
    return `<div style="padding:40px;text-align:center;color:var(--text-muted);">Loading bursars...</div>`;
  }

  return `
  <div class="page-header">
    <h1 class="page-title">Bursars</h1>
    <button class="btn btn-primary" onclick="showBursarModal()">+ Add Bursar</button>
  </div>

  <div class="card" style="margin-bottom:16px;padding:14px 18px;background:linear-gradient(135deg,#f0fdf4,#e8f5e9);border-left:4px solid #1a6e3c;">
    <div style="font-weight:700;color:#1a6e3c;margin-bottom:4px;">About Bursar Accounts</div>
    <div style="font-size:13px;color:#2d6a4f;">Bursars can log in, create receipts, view all receipts, and share receipt links. They cannot access student data, results, or settings.</div>
  </div>

  <div class="card" style="padding:0;overflow:hidden;">
    ${bursars.length === 0
      ? `<div style="padding:32px;text-align:center;color:var(--text-muted);">No bursar accounts yet. Add one to get started.</div>`
      : `<table class="data-table">
          <thead><tr>
            <th>Name</th><th>Username</th><th>Created</th><th style="width:140px;"></th>
          </tr></thead>
          <tbody>
            ${bursars.map(b => `
            <tr>
              <td style="font-weight:600;">${b.name || b.username}</td>
              <td style="color:var(--text-muted);">${b.username}</td>
              <td style="color:var(--text-muted);font-size:12px;">${b.createdAt ? new Date(b.createdAt).toLocaleDateString() : ''}</td>
              <td>
                <button class="btn btn-ghost btn-sm" onclick="showBursarModal('${b.id}','${b.username}','${(b.name||b.username).replace(/'/g,"\\'")}')">Edit</button>
                <button class="btn btn-ghost btn-sm" style="color:#dc2626;" onclick="deleteBursar('${b.id}')">Delete</button>
              </td>
            </tr>`).join('')}
          </tbody>
        </table>`}
  ...
  <!-- Bursar Modal -->
  <div id="bursar-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:1000;align-items:center;justify-content:center;">
    <div class="card" style="width:400px;padding:28px;">
      <h3 id="bursar-modal-title" style="margin-bottom:20px;color:#1a6e3c;">Add Bursar</h3>
      <input type="hidden" id="bursar-id" value="" />
      <div class="form-group" style="margin-bottom:14px;">
        <label>Full Name</label>
        <input id="bursar-name" type="text" class="input" placeholder="e.g. Mrs. Adeyemi" />
      </div>
      <div class="form-group" style="margin-bottom:14px;">
        <label>Username</label>
        <input id="bursar-username" type="text" class="input" placeholder="e.g. bursar" />
      </div>
      <div class="form-group" style="margin-bottom:20px;">
        <label>Password <span id="bursar-pass-hint" style="font-weight:400;color:var(--text-muted);text-transform:none;">(required for new)</span></label>
        <input id="bursar-password" type="password" class="input" placeholder="Leave blank to keep existing" />
      </div>
      <div id="bursar-error" style="color:#dc2626;font-size:13px;margin-bottom:12px;min-height:16px;"></div>
      <div style="display:flex;gap:10px;">
        <button class="btn btn-primary" onclick="saveBursar()">Save</button>
        <button class="btn btn-ghost" onclick="closeBursarModal()">Cancel</button>
      </div>
    </div>
  </div>`;
}

function showBursarModal(id='', username='', name='') {
  DB._bursarsLoaded = false;
  document.getElementById('bursar-modal').style.display = 'flex';
  document.getElementById('bursar-modal-title').textContent = id ? 'Edit Bursar' : 'Add Bursar';
  document.getElementById('bursar-id').value       = id;
  document.getElementById('bursar-username').value = username;
  document.getElementById('bursar-name').value     = name;
  document.getElementById('bursar-password').value = '';
  document.getElementById('bursar-error').textContent = '';
  document.getElementById('bursar-pass-hint').textContent = id ? '(leave blank to keep existing)' : '(required)';
}

function closeBursarModal() {
  document.getElementById('bursar-modal').style.display = 'none';
}

async function saveBursar() {
  const id       = document.getElementById('bursar-id').value;
  const username = document.getElementById('bursar-username').value.trim();
  const name     = document.getElementById('bursar-name').value.trim();
  const password = document.getElementById('bursar-password').value;
  const errEl    = document.getElementById('bursar-error');
  errEl.textContent = '';

  if (!username) { errEl.textContent = 'Username is required'; return; }
  if (!id && !password) { errEl.textContent = 'Password is required for new bursar'; return; }

  try {
    if (id) {
      await DB.updateBursar(id, { username, name, password: password || undefined });
    } else {
      await DB.createBursar({ username, password, name });
    }
    DB._bursarsList  = null;
    DB._bursarsLoaded = false;
    closeBursarModal();
    navigate('bursars');
  } catch(e) {
    errEl.textContent = e.message || 'Failed to save bursar';
  }
}

async function deleteBursar(id) {
  if (!confirm('Delete this bursar account?')) return;
  await DB.deleteBursar(id);
  DB._bursarsList  = null;
  DB._bursarsLoaded = false;
  navigate('bursars');
}
